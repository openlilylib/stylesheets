...
% Storage for all stylesheet data
\registerOption stylesheets.propsets #'()
...
#(with-required-options define-void-function (path)(symbol-list?)
   '(flexible)
   (let* ((parent (append '(stylesheets propsets) path))
          (prop-path (append parent '(props)))
          (propset '()))

     (registerOption prop-path '())
     (setChildOption parent 'stylesheets '())
     (for-each
      (lambda (prop)
        (if (property-definition? prop)
             ...
             (oll:error
              (format "Error in \\setupPropset.
Property definition (list with name, predicate and default) expected, got\n\n~a" prop))
             ))
      props)))
